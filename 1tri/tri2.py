...
# use with python 3
# draw the polygon and triangulation with turtle. 
...
def drawVertices():
    ...
    firstV=vertices[0].v
    up()
    setpos(firstV[0],firstV[1])
    n=len(vertices)
    down()
    for i in range(1, n):
        v=vertices[i].v
        goto(v[0],v[1])
    goto(firstV[0],firstV[1])
    up()
    
def drawDiagonal( a, b ):  #a,b are vStructures    
    goto(a.v[0], a.v[1])
    down()
    goto(b.v[0], b.v[1])
    up()
...
def ReadVertices2():
    ...
    v=tVertexStructure(vnum, v6, False)  
    vnum+=1
    Add(v)
    ...
    while n > 3:  
        ...
        for v2 in vertices:
            if v2.ear:
                ...
                # Cut off the ear v2 */                
                vertices.remove(v2)
                print('Removed a vertex:{0}, {1}'.format(v2.v[0],v2.v[1]))
                n-=1;                
